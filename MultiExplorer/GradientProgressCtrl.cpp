...
void CGradientProgressCtrl::OnPaint() 
{
	...
	if ( m_nCurrentPosition >= m_nMaxSteps-1 )
	{
		int i  = 1;
	}
	// If the current positionis  invalid then we should fade into the  background
	if (m_nCurrentPosition <= 0 || m_nCurrentPosition >= m_nMaxSteps-1)
	{
		CRect rect;
		GetClientRect(rect);
		CBrush brush;
		brush.CreateSolidBrush(m_crBKColor);
		dc.FillRect(&rect, &brush);
		VERIFY(brush.DeleteObject());

		dc.SetTextColor(m_clrText);
		dc.SetBkMode(TRANSPARENT);
		dc.DrawText(m_strMessage, &rect, DT_VCENTER |  DT_CENTER | DT_SINGLELINE);
	...
	float maxWidth((float)m_nCurrentPosition/m_nMaxSteps * (float)rectClient.right);
	...
	if (m_bShowPercent)
	{
		strMessage.Format("%s %d%%", m_strMessage, m_nCurrentPosition*100/m_nMaxSteps);
	}
	else
	{
	...
	dc.SetTextColor(m_clrText);
	dc.SetBkMode(TRANSPARENT);
	dc.DrawText(strMessage, &rectClient, DT_VCENTER |  DT_CENTER | DT_SINGLELINE);
...
void CGradientProgressCtrl:: SetRange(int nLower, int nUpper)
{
	...
	if ( nUpper < m_nMaxSteps )
	{
		m_nMaxSteps = nUpper;
	}
	else
	{
		m_nMaxSteps = min(500,nUpper);
	}
	CProgressCtrl::SetRange(0, m_nMaxSteps);
}
...
{
	int nNewPos = (nPos-m_nLower)*m_nMaxSteps/(m_nUpper-m_nLower);
	if ( nNewPos != m_nCurrentPosition )
	{
		m_nCurrentPosition = nNewPos; 
		return (CProgressCtrl::SetPos(nNewPos));
	}
...
void CGradientProgressCtrl::ShowMessage(CString strMessage, COLORREF crBKColor)
{
