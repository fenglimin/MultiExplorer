            ...
            while (S_OK==lpe->Next(1, &lpi, &ulFetched))
            {
                ...
                if (ulAttrs & (SFGAO_HASSUBFOLDER | SFGAO_FOLDER))
                {
                   ...
                   if (ulAttrs & SFGAO_FOLDER)
                   {
					  ...
					  BOOL bAddToTree = TRUE;
					  //if ( hParent != TVI_ROOT && GetParentItem(hParent) == NULL )
					  //{
						 // CString strParentName = GetItemText(hParent);
						 // if ( strParentName == "我的电脑" || strParentName == "My Computer")
						 // {
							//  CString strName = szBuff;
							//  if ( strName == _T("控制面板") || strName == _T("Control Panel") )
							//  {
							//	  bAddToTree = FALSE;
							//  }
						 // }
					  ...
					  if ( hParent == TVI_ROOT)
					  {
						  CString strText = tvins.item.pszText;
						  if ( !IsMyComputerText(strText) )
							  bAddToTree = FALSE;
					  }
...
void CShellTree::TunnelTree(CString szFindPath)
{
	...
	CString strText = GetItemText ( subNode );
	while ( !IsMyComputerText(strText) )
	{
		subNode = GetNextSiblingItem ( subNode );
		if (subNode == NULL)
			return;
		strText = GetItemText ( subNode );
	}

	Select ( subNode, TVGN_CARET );
...
BOOL CShellTree::IsMyComputerText(CString strText)
{
	return strText == "我的电脑" || strText == "My Computer" || strText == "计算机" || strText == "Computer" || strText == "This PC" || strText == "这台电脑";
}